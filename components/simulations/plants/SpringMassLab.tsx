...
function SpringParameters({ config, onChange, onCountChange }: { config: SpringMassConfig; onChange: (config: SpringMassConfig) => void; onCountChange: (count: number) => void }) {
  const updateMass = (index: number, value: number) => onChange({ ...config, masses: config.masses.map((mass, current) => current === index ? positiveNumber(value, .05) : mass) });
  const updateInitial = (index: number, value: number) => onChange({ ...config, initialDisplacements: config.initialDisplacements.map((item, current) => current === index ? finiteNumber(value) : item) });
  ...
  return <div className="plant-parameters"><label className="plant-select"><span>质量块数量</span><select value={config.masses.length} onChange={(event) => onCountChange(Number(event.target.value))}><option value={1}>1 个</option><option value={2}>2 个</option><option value={3}>3 个</option></select></label><div className="plant-parameter-grid">{config.masses.map((mass, index) => <div className="plant-parameter-unit" key={index}><strong>质量块 {index + 1}</strong><PlantNumberField label="质量 / kg" value={mass} onChange={(value) => updateMass(index, value)} /><PlantNumberField label="初始位移 / m" value={config.initialDisplacements[index]} onChange={(value) => updateInitial(index, value)} /></div>)}</div><div className="spring-links">{config.links.map((link, index) => <div className="spring-link-editor" key={link.id}><strong>{link.left < 0 ? `墙 — 质量块 ${link.right + 1}` : `质量块 ${link.left + 1} — ${link.right + 1}`}</strong><label className="plant-check"><input type="checkbox" checked={link.springEnabled} disabled={link.springEnabled && config.links.filter((item) => item.springEnabled || item.damperEnabled).length === 1 && !link.damperEnabled} onChange={(event) => updateLink(index, { springEnabled: event.target.checked })} /><span>弹簧</span></label>{link.springEnabled && <PlantNumberField label="k / N·m⁻¹" value={link.spring} onChange={(value) => updateLink(index, { spring: positiveNumber(value, .01) })} />}<label className="plant-check"><input type="checkbox" checked={link.damperEnabled} disabled={link.damperEnabled && config.links.filter((item) => item.springEnabled || item.damperEnabled).length === 1 && !link.springEnabled} onChange={(event) => updateLink(index, { damperEnabled: event.target.checked })} /><span>阻尼器</span></label>{link.damperEnabled && <PlantNumberField label="c / N·s·m⁻¹" value={link.damper} onChange={(value) => updateLink(index, { damper: positiveNumber(value, .01) })} />}</div>)}</div><label className="plant-select"><span>外力作用对象</span><select value={config.forceTarget} onChange={(event) => onChange({ ...config, forceTarget: Number(event.target.value) })}>{config.masses.map((_, index) => <option key={index} value={index}>质量块 {index + 1}</option>)}</select></label></div>;
}
